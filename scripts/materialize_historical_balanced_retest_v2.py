...
FINALIZE_SEED = "MOATRADER_V2_BALANCED_RETEST_1_FINAL_20260822"
...
def _read_packets(path: Path) -> list[PairedAxisPacket]:
    ...
    if len({row.packet_id for row in rows}) != len(rows):
        raise ValueError("Balanced retest packet IDs must be unique")
    return rows
...
def materialize_balanced_retest_human_gold(
    *,
    candidate_build: Path,
    review_decisions: Path,
    output: Path,
    minimum_per_axis_stratum: int = 5,
    seed: str = FINALIZE_SEED,
) -> dict[str, Any]:
    if output.exists() and any(output.iterdir()):
        raise FileExistsError(f"output directory must be new or empty: {output}")
    if minimum_per_axis_stratum < 5:
        raise ValueError("Balanced retest requires at least five cases per axis/stratum")
    ...
    template_path = candidate_build / "balanced-retest-human-gold-template.csv"
    hint_path = candidate_build / "balanced-retest-selection-hints.jsonl"
    manifest = _read_json(manifest_path)
    if manifest.get("status") != (
        "V2_BALANCED_RETEST_1_CANDIDATES_PREPARED_OUTCOME_BLIND"
    ):
        raise ValueError("Balanced retest candidates are not outcome-blind and ready")
    _require_downstream_closed(manifest, "Balanced retest candidate manifest")
    for key in (
        "selection_used_parser_classifications",
        "selection_used_post_test_disagreement_rows",
    ):
        if manifest.get(key) is not False:
            raise ValueError("Balanced retest selection was contaminated by prior model results")
    if manifest.get("prior_human_labels_accepted_as_retest_gold") is not False:
        raise ValueError("prior HUMAN labels cannot be reused as Balanced retest gold")
    if manifest.get("fresh_independent_human_review_required") is not True:
        raise ValueError("Balanced retest must require fresh independent HUMAN review")
    if manifest.get("first_balanced_test_remains_consumed") is not True:
        raise ValueError("first Balanced LOCKED test must remain consumed")
    expected_hashes = {
        "balanced_retest_candidate_packet_sha256": sha256_file(packet_path),
        "balanced_retest_human_gold_template_sha256": sha256_file(template_path),
        "balanced_retest_selection_hint_sha256": sha256_file(hint_path),
    }
    for key, value in expected_hashes.items():
        if manifest.get(key) != value:
            raise ValueError(f"Balanced retest candidate artifact changed: {key}")
    ...
    if payload.get("reviewer") != "HUMAN":
        raise ValueError("Balanced retest review decisions must be tagged exactly HUMAN")
    reviewer_name = str(payload.get("human_reviewer_name") or "").strip()
    if not reviewer_name:
        raise ValueError("Balanced retest requires the actual HUMAN reviewer name")
    if payload.get("attestation") != "YES":
        raise ValueError("Balanced retest HUMAN attestation must be exactly YES")
    review_date = str(payload.get("review_date") or "").strip()
    if not review_date:
        raise ValueError("Balanced retest requires a review date")
    _require_downstream_closed(payload, "Balanced retest review decisions")
    decisions = payload.get("decisions")
    if not isinstance(decisions, list):
        raise ValueError("Balanced retest review decisions must contain a decisions list")

    rows: dict[str, dict[str, str]] = {}
    status_counts: dict[str, int] = {}
    buckets: dict[tuple[Any, str], list[PairedAxisPacket]] = defaultdict(list)
    for number, raw in enumerate(decisions, start=1):
        ...
        if packet_id in rows:
            raise ValueError(f"duplicate Balanced retest decision: {packet_id}")
        packet = packet_lookup.get(packet_id)
        if packet is None:
            raise ValueError(f"review decision is outside Balanced retest 1: {packet_id}")
        ...
        row = {
            ...
            "gold_split": CANDIDATE_SPLIT,
            "gold_contract_version": CANDIDATE_CONTRACT,
            "reviewer": "HUMAN",
            "review_notes": notes,
        }
        ...
        validate_classification_grounding(label, packet)
        rows[packet_id] = row
        buckets[(packet.axis, _stratum(label))].append(packet)

    if set(rows) != set(packet_lookup):
        missing = sorted(set(packet_lookup) - set(rows))
        extra = sorted(set(rows) - set(packet_lookup))
        raise ValueError(
            "HUMAN decisions must exactly cover Balanced retest candidates; "
    ...
    for axis in SEMANTIC_AXES:
        available_counts[axis.value] = {}
        selected_counts[axis.value] = {}
        for stratum in STRATA:
            candidates = sorted(
                buckets[(axis, stratum)], key=lambda packet: _selection_key(packet, seed)
            )
            available_counts[axis.value][stratum] = len(candidates)
            if len(candidates) < minimum_per_axis_stratum:
                raise ValueError(
                    f"insufficient fresh HUMAN {axis.value}/{stratum} Balanced retest cases"
                )
            chosen = candidates[:minimum_per_axis_stratum]
            selected.extend(chosen)
            selected_counts[axis.value][stratum] = len(chosen)
    selected.sort(key=lambda packet: (packet.axis.value, packet.packet_id))

    output.mkdir(parents=True, exist_ok=True)
    reviewed_gold_path = output / "balanced-retest-reviewed-candidate-human-gold.csv"
    selected_packet_path = output / "balanced-retest-packets.jsonl"
    selected_gold_path = output / "balanced-retest-human-gold.csv"
    _write_gold(reviewed_gold_path, [rows[packet.packet_id] for packet in packets])
    _write_packets(selected_packet_path, selected)
    selected_rows: list[dict[str, str]] = []
    for packet in selected:
        row = dict(rows[packet.packet_id])
        row["gold_split"] = RETEST_SPLIT
        row["gold_contract_version"] = RETEST_CONTRACT
        selected_rows.append(row)
    _write_gold(selected_gold_path, selected_rows)

    result = {
        "schema_version": "moatrader-v2-balanced-retest-human-gold-materialization/1",
        "status": "V2_BALANCED_RETEST_1_HUMAN_GOLD_MATERIALIZED_OUTCOME_BLIND",
        "reviewer": "HUMAN",
        ...
        "available_stratum_counts": available_counts,
        ...
        "balanced_retest_candidate_packet_sha256": sha256_file(packet_path),
        "reviewed_candidate_human_gold_sha256": sha256_file(reviewed_gold_path),
        "balanced_retest_packet_sha256": sha256_file(selected_packet_path),
        "balanced_retest_human_gold_sha256": sha256_file(selected_gold_path),
        "source_spans_materialized_from_fresh_human_anchors": True,
    ...
    _write_json(output / "balanced-retest-human-gold-materialization-manifest.json", result)
    return result


def freeze_balanced_retest_measurement(
    *,
    ...
    if output.exists():
        raise FileExistsError(f"Balanced retest measurement freeze already exists: {output}")
    candidate_manifest_path = candidate_build / "balanced-retest-preparation-manifest.json"
    materialization_path = (
        human_gold_build / "balanced-retest-human-gold-materialization-manifest.json"
    )
    packet_path = human_gold_build / "balanced-retest-packets.jsonl"
    human_gold_path = human_gold_build / "balanced-retest-human-gold.csv"
    ...
    for payload, description in (
        ...
        (candidate, "Balanced retest candidate manifest"),
        (materialization, "Balanced retest HUMAN materialization"),
    ):
    ...
    if freeze.get("schema_version") != "moatrader-historical-evidence-parser-freeze-v2/2":
        raise ValueError("Balanced retest must inherit the original semantic V2 freeze")
    ...
    if candidate.get("status") != (
        ...
        raise ValueError("Balanced retest candidate preparation is invalid")
    if candidate.get("parser_freeze_sha256") != sha256_file(parser_freeze_manifest):
        raise ValueError("Balanced retest candidates do not inherit the root freeze")
    if materialization.get("status") != (
        "V2_BALANCED_RETEST_1_HUMAN_GOLD_MATERIALIZED_OUTCOME_BLIND"
    ):
        raise ValueError("Balanced retest HUMAN gold is not materialized")
    if materialization.get("candidate_preparation_manifest_sha256") != sha256_file(
        candidate_manifest_path
    ):
        raise ValueError("Balanced retest HUMAN gold does not match candidate preparation")
    if materialization.get("balanced_retest_packet_sha256") != sha256_file(packet_path):
        raise ValueError("Balanced retest packets changed before freeze")
    if materialization.get("balanced_retest_human_gold_sha256") != sha256_file(
        human_gold_path
    ):
        raise ValueError("Balanced retest HUMAN gold changed after materialization")

    result = {
        "schema_version": "moatrader-historical-evidence-parser-retest-freeze-v2/1",
        "status": "V2_BALANCED_RETEST_1_FROZEN_AWAITING_SINGLE_USE_TEST",
        ...
        **expected,
        ...
        "human_gold_materialization_manifest_sha256": sha256_file(materialization_path),
        "balanced_retest_packet_sha256": sha256_file(packet_path),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Materialize and freeze independent Balanced LOCKED Retest 1 HUMAN gold."
    )
    ...
    materialize.add_argument("--minimum-per-axis-stratum", type=int, default=5)
    materialize.add_argument("--seed", default=FINALIZE_SEED)
    ...
    if args.command == "materialize":
        result = materialize_balanced_retest_human_gold(
        ...
        result = freeze_balanced_retest_measurement(
