...
SCHEMA_VERSION = "expanded-valuation-signal-audit/1"
    ...
    *,
    ...
    if method == ValuationMethod.RIM:
        ...
        try:
            ...
            return None, f"RIM_ERROR:{type(exc).__name__}:{exc}"
    if method in {
        ...
        if dcf_input is None:
            return None, "MISSING_FCFF_INPUT"
        try:
            ...
            return None, f"FCFF_ERROR:{type(exc).__name__}:{exc}"
    return None, f"METHOD_INPUT_NOT_AVAILABLE:{method.value}"


def run(args: argparse.Namespace) -> dict[str, Any]:
    for path in (args.dates, args.universe, args.base_root, args.output):
    ...
    for as_of in dates:
        ...
        for code, universe_row in sorted(universe.items()):
            ...
            valuation, valuation_reason = _valuation_for(
                method=route.primary_method,
                metrics=metrics,
                universe=universe_row,
                dcf_input=dcf_input,
                provenance=profile.provenance,
            ) if route.applicability.status.value == "ELIGIBLE" else (None, ";".join(route.applicability.reason_codes))
            price = None
            ...
            signals.append(
                {
                    ...
                    "possible_pass": int(
                        bool(
                            valuation
                            and valuation.applicability.status.value == "ELIGIBLE"
                            and valuation.metadata.get("screening_eligible", True)
                        )
                    ),
    ...
    write_csv(
        ...
        [
            ...
            "applicability_status", "missing_fields", "profile_sha256", "valuation_generated",
            "valuation_reason",
        ],
    )
    write_csv(
        ...
        signals,
        [
            "date", "ticker", "method", "economic_archetype", "sector",
            ...
            "valuation_warning_count", "possible_pass", "valuation_reason",
        ],
    )
    coverage = {
...
def main() -> int:
    ...
    for name in ("dates", "universe", "base_root", "output", "pit_sector_map", "development_sector_map"):
